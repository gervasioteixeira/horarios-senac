...
<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <div>
        <h2 class="text-xl font-semibold text-slate-800">Cursos</h2>
        <p class="text-sm text-slate-500">Cadastro de cursos e carga horária total.</p>
      </div>
      <button
        type="button"
      ...
      <table class="w-full min-w-[640px] text-left text-sm">
        ...
        <tbody class="divide-y divide-slate-100">
          <tr v-if="sortedCourses.length === 0">
            <td colspan="5" class="px-4 py-6 text-center text-slate-400">Nenhum curso cadastrado ainda.</td>
          </tr>
          <tr v-for="course in sortedCourses" :key="course.id">
            <td class="px-4 py-3 font-medium text-slate-800">{{ course.name }}</td>
            <td class="px-4 py-3 max-w-xs truncate text-slate-600" :title="course.description">{{ course.description || "—" }}</td>
            <td class="px-4 py-3 text-slate-600">{{ course.totalWorkloadHours }}h</td>
            <td class="px-4 py-3">
              <span
                class="rounded-full px-2 py-0.5 text-xs font-medium"
                :class="course.active ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-500'"
              >
