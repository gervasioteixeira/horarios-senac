...
| Identificação da versão em execução (suporte / checar página em cache) | Linha discreta no rodapé do `App.vue`: "Versão X.Y.Z · dd/mm/aaaa hh:mm" (fuso de Brasília), com commit no tooltip. Valores injetados no build por `define` em `vite.config.ts` (`__APP_VERSION__` do `package.json`, `__APP_COMMIT__` via `git rev-parse --short HEAD`, `__APP_BUILT_AT__`), tipados em `src/env.d.ts` e formatados em `src/constants/version.ts`. **Para uma release nova, suba a versão no `package.json`** (`npm version <x.y.z> --no-git-tag-version`); se o rodapé mostrar versão/data antigas, o navegador está com a página em cache |
